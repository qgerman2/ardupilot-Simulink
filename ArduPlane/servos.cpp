...
void Plane::servos_output(void)
{
    ...
    // simulink
    simulink.run(AP_Simulink::nav_inputs {
        control_mode->mode_number(),
        mission.get_current_nav_cmd().id,
        // At this point, nav_roll_cd includes stall prevention limits set by Plane::update_load_factor.
        // It also considers the maximum bank limit set in the parameters.
        // This value is generated by the L1 controller.
        radians(nav_roll_cd*0.01f),
        // Pitch and throttle come from the TECS controller.
        // Pitch is limited by the minimum down pitch angle allowed in the parameters.
        radians(nav_pitch_cd*0.01f),
        TECS_controller.get_throttle_demand(),
    });
